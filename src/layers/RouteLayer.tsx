...
export default function RouteLayer() {
  ...
  useEffect(() => {
    ...
    const runUpdate = () => {
      ...
      const trailHasLine = trailGeojson.features.length > 0
      const trailFollowActive =
        snapToTrailEnabled &&
        trailHasLine &&
        trailRoute.legs.some((leg) => leg.mode === 'trail' && leg.points.length >= 3)

      /** Pin-to-pin line is mandatory whenever 2+ waypoints exist; hide only when trail line is live. */
      const routeGeojson =
        trailFollowActive && trailHasLine ? EMPTY_FC : pinGeojson
      ...
      if (source) {
        ...
        const ensure = () => {
          ...
          ensureRouteLayers(routeGeojson)
          const created = map.getSource(ROUTE_SOURCE_ID) as maplibregl.GeoJSONSource | undefined
          created?.setData(routeGeojson)
        }
      ...
      if (trailHasLine) {
        ...
        ;(map.getSource(TRAIL_ROUTE_SOURCE_ID) as maplibregl.GeoJSONSource)?.setData(EMPTY_FC)
      }

      reportRouteLayerObservation({
        waypointCount: visibleWaypoints.length,
        pinLineFeatureCount: routeGeojson.features.length,
        trailLineFeatureCount: trailGeojson.features.length,
      })
    }
